...
from src.analyzer import full_word_analysis, get_unique_synonyms, filter_synonyms_by_corpus
...
if search_word:
    ...
    search_word = morph.parse(search_word)[0].normal_form # Лемматизируем для надежности
    ...
    if not results:
        ...
        filtered_synonyms = filter_synonyms_by_corpus(synonyms, df_filtered) # Фильтруем по корпусу
        ...
        if synonyms:
            ...
            st.write(f"Синонимы, найденные в корпусе: {', '.join(filtered_synonyms)}")
        else:
