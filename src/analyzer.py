...
def prepare_llm_prompt(target_word, synonyms, synonyms_filtered, syn_proximity, neighbors_for_synonyms, total_occurrences=0, year_dist=None):
    ...
    prompt = f"""
Ты работаешь с корпусными данными поэтических текстов. Тебе предоставлены числовые данные о слове "{target_word.upper()}" и его синонимах. Твоя задача — написать короткий читаемый комментарий, основанный ТОЛЬКО на этих данных. Не добавляй сведений, которых нет в данных ниже.

--- КАК ЧИТАТЬ ДАННЫЕ ---

"Семантическая близость" (общий корпус) — насколько слово близко к "{target_word}" в русской литературе в целом. Чем выше значение, тем слова ближе по смыслу в стандартном употреблении.

"Индекс контекстуальной близости" — показывает, насколько часто слово встречается рядом с "{target_word}" в поэтических текстах. Учитывает расстояние: слова вплотную дают больший вклад, чем слова через несколько позиций. Переход через границу строки или предложения снижает вклад. Таким образом, высокий индекс = слово регулярно оказывается в непосредственном контекстуальном окружении целевого слова.

--- ДАННЫЕ ---

Всего употреблений слова "{target_word}" в выбранном периоде: {total_occurrences}

Динамика по годам (год: количество употреблений):
{year_dist_str}
{f'Пик употреблений: {peak_year} год ({peak_count} вхождений)' if peak_year else ''}

Синонимы из общего корпуса (семантическая близость):
{', '.join([f"{syn} ({score:.4f})" for syn, score in synonyms])}

Синонимы, встречающиеся в текстах, и их индекс контекстуальной близости к "{target_word}":
{synonyms_str if synonyms_str else 'нет'}

Синонимы, полностью отсутствующие в текстах:
{', '.join([syn for syn, score in synonyms if syn not in synonyms_filtered]) or 'нет'}

Контекстуальное окружение каждого встречающегося синонима — топ-10 слов-соседей с их индексами:
{neighbors_for_synonyms_str}

--- ФОРМАТ ОТВЕТА ---

Напиши четыре коротких абзаца без вступлений, без упоминания меток вроде "[A]" или "пункт 4". Пиши для читателя, который видит только твой текст — без таблиц и сырых данных.

**Частотность и динамика.** Сколько раз слово встречается в выбранном периоде? Есть ли выраженный пик или равномерное распределение по годам? Назови конкретные годы и числа, если они показательны.

**Синонимы в текстах.** Какие из синонимов появляются рядом с "{target_word}" чаще всего (высокий индекс), а какие — почти не попадают в его окружение (низкий индекс)? Назови конкретные слова и значения.

**Различия в окружении.** Чем отличается контекст разных синонимов? Какие слова-соседи характерны для одного синонима, но не для другого? Опирайся на конкретные леммы и их индексы.

**Отсутствующие синонимы.** Какие слова, близкие к "{target_word}" в общем языке, в текстах не встречаются совсем? Насколько они были близки по общему корпусу?

**Что выделяется в данных.** 2–3 конкретных наблюдения с числами, которые кажутся наиболее значимыми или неожиданными на основе этих данных.
"""
    return prompt
